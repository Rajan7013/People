...
import { House, Users, CalendarCheck, LogOut, AlignJustify } from "lucide-react";
...
export default function TeamLeadLayout({
    ...
    // Protect the route - allow 'team_lead'
    useEffect(() => {
        if (!isLoading) {
            if (!isAuthenticated) {
                router.push("/login");
    ...
    if (!isAuthenticated || user?.role !== 'team_lead') {
        return null;
    }

    const navItems = [
        { name: "Dashboard", href: "/team-lead/dashboard", icon: House },
        { name: "My Team", href: "/team-lead/team", icon: Users },
        { name: "Attendance", href: "/team-lead/attendance", icon: CalendarCheck },
    ];
        ...
        <div className="flex h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            `}>
                <div className="h-16 flex items-center px-6 border-b border-gray-200 dark:border-slate-800">
                    <h1 className="text-xl font-bold text-gray-900 dark:text-white">PeopleOS <span className="text-xs font-normal text-gray-500 uppercase">Lead</span></h1>
                </div>
